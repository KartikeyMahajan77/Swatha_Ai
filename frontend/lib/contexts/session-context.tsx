...
export function SessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkSession = async () => {
    try {
      ...
      if (storedRole === "admin" || storedRole === "therapist") {
        setUser(null);
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener(AUTH_ROLE_CHANGE_EVENT, handleAuthChange);
    window.addEventListener("storage", handleAuthChange);

    return () => {
      window.removeEventListener(AUTH_ROLE_CHANGE_EVENT, handleAuthChange);
