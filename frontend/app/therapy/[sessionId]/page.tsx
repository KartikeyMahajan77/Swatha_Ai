...
export default function TherapyPage() {
  ...
  const handleSubmit = async (
    ...
    const currentMessage = (messageOverride ?? message).trim();
    ...
    if (!currentMessage || isTyping || isChatPaused || !currentSessionId) {
    ...
    try {
      ...
      // Send message to API
      const response = await sendChatMessage(currentSessionId, currentMessage);
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content: isAuthError
            ? "Please sign in again before continuing this chat. Your session token is missing or expired."
            : `I apologize, but I'm having trouble connecting right now. ${errorMessage}`,
          timestamp: new Date(),
        },
      ]);
      setIsTyping(false);
  ...
  const handlePasteMessage = (
    ...
    insertTextIntoMessage(event.currentTarget, pastedText);
  };
  ...
  useEffect(() => {
    ...
    const handleNativePaste = (event: ClipboardEvent) => {
      ...
      if (!pastedText || input.disabled) {
        return;
      }

      event.preventDefault();
      insertTextIntoMessage(input, pastedText);
    };
  ...
  const handleSuggestedQuestion = async (text: string) => {
    ...
    handleSubmit(event, text, activeSessionId || undefined);
  };
